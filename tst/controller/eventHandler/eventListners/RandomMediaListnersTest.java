...
public class RandomMediaListnersTest {
    ...
    <T extends MediaContentUploadable> void SAVE_JOS_LISTNER_TEST() {
        ...
        this.controller.start("save " + ((T)((MediaContentUploadable) this.zufaelligErzeuger.getMediaList().get(0))).getAddr());
        this.model.löschen((T)this.zufaelligErzeuger.getMediaList().get(0));
        assertTrue(this.model.auflisten().size() == 0);
        this.controller.setCurrentMode(":p");
        this.controller.start("load " + ((T)((MediaContentUploadable) this.zufaelligErzeuger.getMediaList().get(0))).getAddr());
        assertTrue(this.model.auflisten().size() == 1);
    }
}
